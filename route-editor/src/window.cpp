...
Window::Window(tkg::ConfigFile &conf) : QMainWindow()
{
    ...
    */
    ...
    w_table   = new QTableWidget(0,11);

    QStringList header;
    std::vector<std::string> table_text = tkg::parseArray(conf["Layout"]["table-text"]);
    for(uint i = 0; i < table_text.size(); ++i)
    {
        header << tr( table_text[i].c_str() );
    }

    int table_width = 0;
    std::vector<std::string> table_size = tkg::parseArray(conf["Layout"]["table-size"]);
    for(uint i = 0; i < table_text.size(); ++i)
    {
        int width = tkg::parseInt(table_size[i]);
        table_width += width;
        w_table->setColumnWidth(i, width);
    }

    w_table->setFixedWidth(table_width);
